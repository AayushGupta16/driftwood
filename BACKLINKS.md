...
| crunchbase.com/organization/driftwood-driftwood-sh | 2026-07-20 | self-serve listing (medium no-social-proof blurb per LISTINGS.md) | nofollow (assumed, unverified) | https://driftwood.sh | Profile URL VERIFIED live 2026-07-21 via search index (DuckDuckGo result, snippet matches our blurb verbatim). Direct fetch of the profile returned 403 (bot-blocked), so the on-page website link is UNVERIFIED — confirm link presence + rel attribute from a browser session. |
