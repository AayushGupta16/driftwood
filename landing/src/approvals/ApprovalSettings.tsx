...
import { MODE_LABELS, MODE_DESCRIPTIONS, type ApprovalMode, type ApprovalPolicy, type Reviewer } from './model';
...
export default function ApprovalSettings() {
 ...
 useEffect(() => {
  ...
  getPolicy(controller.signal).then((p) => {if (!controller.signal.aborted) {setPolicy(p);setError(null);}}).catch((e) => {if (!controller.signal.aborted) setError(e.message);});
 ...
 {policy && <form onSubmit={async (event) => {event.preventDefault();if (!canWrite || busy) return;setBusy(true);setError(null);setSaved(false);try {setPolicy(await savePolicy(policy));setSaved(true);}catch(e){setError(e instanceof Error ? e.message : 'Could not save settings.');}finally{setBusy(false);}}}>
