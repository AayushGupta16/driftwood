...
import { MODE_LABELS, MODE_DESCRIPTIONS, type ApprovalPolicy } from '../approvals/model';
...
import { filterMessages, statusLabel, type InboxFilter } from './model';
import './inbox.css';
const FILTERS = [['queued','Queued'],['pending','Pending'],['sent','Sent'],['reply','Replies']] as const;
...
export default function Inbox() {
 ...
 return <section className="inbox-page">
  ...
  <div className="inbox-approval-banner"><div><strong>{policy ? MODE_LABELS[policy.mode] : policyError ? 'Approval settings unavailable' : 'Loading approval settings…'}</strong><p>{policy ? MODE_DESCRIPTIONS[policy.mode] : policyError ?? 'Checking who reviews your outreach.'}</p></div><a href={withMockMode('/dashboard/settings?tab=approvals')}>{canWrite ? 'Change approval settings' : 'View approval settings'}</a></div>
  ...
  {!busy || rows.length > 0 ? <div className="inbox-layout">
   <div className="inbox-list" aria-label="Messages">{filtered.length === 0 ? <div className="inbox-empty"><h2>No matching messages</h2><p>{Object.keys(data.errors).length ? 'Some activity is unavailable. Retry the failed sources above.' : more.length ? 'Load more activity or change the filters.' : 'Sent and queued outreach will appear here alongside replies.'}</p><a href={withMockMode('/dashboard/campaigns')}>View campaigns →</a></div> : filtered.map((row) => <button key={row.id} className={current === row ? 'is-active' : ''} onClick={() => {setSelected(row.id);setArmed(null);}} aria-pressed={current === row}><span className="inbox-avatar">{row.name.slice(0,1)}</span><span><strong>{row.name}</strong><small>{row.company ?? 'Company unavailable'} · {row.channel}</small><p>{row.subject ?? row.body ?? 'Message content unavailable'}</p><span className={`inbox-status is-${row.status}`}>{row.status === 'approval' ? row.reviewer === 'customer' ? 'Your team reviews' : row.reviewer === 'driftwood' ? 'Driftwood reviews' : 'Awaiting approval' : statusLabel[row.status]}</span></span></button>)}</div>
   {current ? <article className="inbox-message"><header><span>{current.channel} · {statusLabel[current.status]}</span><h2>{current.subject || (current.status === 'reply' ? 'Reply from your prospect' : 'Outbound message')}</h2><p>{current.status === 'reply' ? 'From' : 'To'} {current.name} · {formatObservedAt(current.at)}</p></header>
    ...
    {current.status === 'approval' && <aside>{current.reviewer === 'customer' ? 'Waiting for your team to approve.' : current.reviewer === 'driftwood' ? 'Waiting for Driftwood to review in our dashboard.' : 'Approval assignment is unavailable.'} This message is not scheduled and has not been sent.</aside>}
    ...
    <footer>{canApprove(current) && <button className="approval-primary" disabled={decisionBusy || busy} onClick={() => void approve([current.reviewId!])}>{armed === JSON.stringify([current.reviewId!]) ? 'Confirm approval' : 'Approve message'}</button>}<p>{current.status === 'reply' ? 'Continue the full conversation in your sending account.' : current.status === 'sent' ? 'This is the message recorded by the sending system as sent.' : 'Only approved messages enter the sending queue. Track their status here.'}</p>{current.status === 'reply' && current.channel === 'Email' && current.email && <a href={`mailto:${encodeURIComponent(current.email)}?subject=${encodeURIComponent(`Re: ${current.subject ?? ''}`)}`}>Compose in email app ↗</a>}</footer>
   </article> : <div className="inbox-empty">Choose a message to see its content and delivery status.</div>}
   {current && <aside className="inbox-context"><h2>Prospect</h2><strong>{current.name}</strong><p>{current.title}</p><p>{current.company}</p><p>{current.email}</p><a href={withMockMode('/dashboard/leads')}>View contacts →</a><hr /><h2>Delivery status</h2><strong>{statusLabel[current.status]}</strong><p>{current.channel}</p><a href={withMockMode('/dashboard/campaigns')}>View campaigns →</a></aside>}
  </div> : null}
